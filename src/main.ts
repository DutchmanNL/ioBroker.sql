...
export class SqlAdapter extends Adapter {
    ...
    async testConnection(msg: ioBroker.Message): Promise<void> {
        ...
        const config: SqlAdapterConfigTyped = msg.message.config;

        config.port = parseInt(config.port as unknown as string, 10) || 0;
        ...
        try {
            ...
            const err = await new Promise<Error | null | undefined>(resolve =>
                client.connect((err?: Error | null): void => {
                    ...
                    client.execute('SELECT 2 + 3 AS x', (err?: Error | null /* , rows, fields */): void => {
                        client.disconnect();
                        resolve(err);
                    });
                }),
    ...
    finish(callback: () => void): void {
        ...
        const finishId = (id: string, done: () => void): void => {
            if (!this.sqlDPs[id]) {
                return done();
            }
        ...
        const startFinish = (attempt: number): void => {
     ...
     */
    _insertValuesIntoDB(queries: string[], id: string, cb?: (err?: Error | null) => void): void {
        this.borrowClientFromPool((err, client) => {
            if (err || !client) {
                this.returnClientToPool(client);
                this.log.error(err?.toString() || 'No client');
     ...
     */
    storeCachedBatch(cb?: ((err?: Error | null) => void) | null): void {
        ...
        const entries: { id: string; inFlightId: string; rowCount: number }[] = [];
        ...
        for (const id in this.sqlDPs) {
            ...
            if (this.sqlDPs[id].list?.length) {
                ...
                statements.push(
                    ...this.sqlFuncs!.insert(
                        this.config.dbname,
                        this.sqlDPs[id].index,
                        rows.map(row => ({ ...row, state: { ...row.state } })),
                    ),
                );
     ...
     */
    _insertValuesBatchIntoDB(
        rawQueries: string[],
        entries: { id: string; inFlightId: string; rowCount: number }[],
        cb?: (err?: Error | null) => void,
    ): void {
        ...
        const requeueOrDrop = (err: Error): void => {
        ...
        this.borrowClientFromPool((err, client) => {
            ...
            const finalizeError = (execErr: Error): void => {
                if (transactional) {
            ...
            client.executeStreamed<IobDataEntryEx & { date?: Date; id?: string }>(
                ...
                (streamErr, streamed) => {
                    ...
                    if (streamErr || aggError) {
                        sendResponse(this, msg, options.id!, options, (streamErr || aggError)!.toString(), startTime);
                        return;
                    }
                    try {
                        if (totalRows === 0) {
    ...
    #getAggregatedDataSql(
        ...
        let internalOptions: InternalHistoryOptions;
        let sql: string;
        try {
            internalOptions = initAggregate(options, options.id ?? undefined, undefined, log);
        ...
        const fallbackToNode = (reason: string): void => {
            ...
            if (reason === 'nulls in range' && !this.streamingUnavailable) {
                ...
                if (!this.nullsFallbackReported) {
                    ...
                    this.log.info(
                        `${logId}: range contains null markers - aggregating in the adapter instead of the database (reported once; enable debug log to see each occurrence)`,
                    );
                }
     ...
     */
    applyDockerMysqlConfig(config: SqlAdapterConfigTyped): void {
     ...
     */
    async createUserInDocker(retries = 30): Promise<void> {
        ...
        if (config.dbtype === 'sqlite' || this.config.host) {
            this.connect(() => {
                ...
                this.getObjectView('system', 'custom', {}, (err, doc) => {
                    ...
                    this.bufferChecker = setInterval(
                        () => this.storeCached(),
                        this.config.writeInterval > 0 ? this.config.writeInterval : 10 * 60000,
                    );
                });
            });
        }
